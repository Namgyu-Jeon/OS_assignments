...
CPU 시간을 읽는 API와 기본 계산식은 과제에서 제시되었습니다. 이번 구현에서는 그 공식을 단순히 한 번 적용하는 데 그치지 않고, 매초 안정적으로 측정하고 장시간 실행해도 기록 공간과 처리량이 늘어나지 않도록 만드는 데 중점을 두었습니다.

가장 신경 쓴 부분은 이동평균 처리입니다. 실행 결과를 계속 벡터에 추가하거나 매초 최근 값을 다시 합산하는 대신 15칸 원형 버퍼와 세 이동합을 사용했습니다. 새 값이 들어올 때 각 범위에서 빠지는 값 하나를 빼고 새 값 하나를 더하므로 5초·10초·15초 평균을 모두 O(1)에 갱신합니다. 원형 버퍼가 한 바퀴 돈 이후의 위치 계산은 16번째 값과 60번째 값까지 결정적인 숫자로 검증했습니다.

CPU 부하 계산에서는 `FILETIME`을 안전하게 64비트로 변환하고, 누적시간 역전, 덧셈 오버플로, 0인 전체 시간과 잘못된 Idle 시간 관계를 먼저 검사했습니다. API 호출이나 계산이 실패한 값은 평균에 넣지 않고, 성공한 경우에만 현재 값을 다음 기준으로 갱신합니다.

측정 일정은 실제 날짜·시간과 분리된 `steady_clock`과 `sleep_until()`으로 관리했습니다. 짧은 출력 시간이 매 주기에 누적되는 것을 줄이고, 시스템이 크게 늦어진 경우에는 놓친 횟수를 한꺼번에 출력하지 않도록 했습니다. 소스는 실행 관리, Windows API 처리, 이동평균 관리로 나누고 비전공자도 이유를 따라갈 수 있도록 한국어 주석을 작성했습니다.

Windows API와 부하 공식 자체는 과제에서 정해진 부분이고 새로운 CPU 측정 방식을 만든 것은 아니므로 100점으로 평가하지 않았습니다. 다만 고정 메모리 이동평균, 일정 관리, 경계값 방어, 오류 처리, 모듈 분리와 실제 저부하·영상 재생·계산 부하 검증까지 완성한 점을 반영해 90점으로 평가했습니다.
